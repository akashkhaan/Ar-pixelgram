...
const CreateGroupPage: React.FC = () => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (avatarFile) {
        try { await uploadGroupAvatar(groupId, avatarFile); }
    ...
    <MobileLayout hideHeader hideNav>
      <div className="min-h-[100dvh] bg-background">
        ...
        <form onSubmit={submit} className="space-y-5 p-4 pb-10">
          <div className="flex items-center gap-3 rounded-xl border border-border bg-card p-3"><label className="relative h-14 w-14 shrink-0 cursor-pointer overflow-hidden rounded-full bg-primary/15 flex items-center justify-center text-primary">{avatarPreview ? <img src={avatarPreview} alt="Group preview" className="h-full w-full object-cover" /> : <Users className="h-7 w-7" />}<span className="absolute inset-x-0 bottom-0 bg-black/55 py-0.5 text-center text-[9px] text-white">Choose</span><input type="file" accept="image/*" className="hidden" onChange={event => { const file = event.target.files?.[0] || null; setAvatarFile(file); setAvatarPreview(file ? URL.createObjectURL(file) : ""); }} /></label><div className="min-w-0 flex-1"><p className="font-medium">Group details</p><p className="text-xs text-muted-foreground">Name is required. Choose a photo from your gallery.</p></div></div>
